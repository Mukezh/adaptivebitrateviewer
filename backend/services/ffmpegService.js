...
import { uploadHLSToS3 } from "./s3Service.js";
...
export const transcodeToHLS = async (inputPath, outputDir, baseFileName) => {
  ...
  for (const res of resolutions) {
    ...
    const cmd = `ffmpeg -i "${inputPath}" \
      -vf scale=${res.size} -c:a aac -ar 48000 -c:v h264 \
      -profile:v main -crf 20 -sc_threshold 0 \
      -g 48 -keyint_min 48 -b:v ${res.bitrate} \
      -maxrate ${res.bitrate} -bufsize ${2 * parseInt(res.bitrate)} \
  ...
  for (const file of files) {
    ...
    await uploadHLSToS3(`hls/${baseFileName}/${file}`, buffer, contentType);
    console.log("uploaded transcoded file : ", file);
  }
